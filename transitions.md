...
The classic. One clip fades out while the next fades in.
...
Clip 1 fades to black, then clip 2 fades in from black.
...
Same as dip-to-black but with white flash.
...
Clip 2 slides in from right, wiping clip 1 away.
...
Clip 2 slides up from the bottom.
...
Clip 2 slides down from the top.
...
Clip 1 zooms in while fading to clip 2.
...
Clip 2 starts zoomed in and pulls back to normal.

```bash
# Use xfade filter (ffmpeg 4.3+)
ffmpeg -i clip1.mp4 -i clip2.mp4 -filter_complex "
  [0:v]trim=duration=5,setpts=PTS-STARTPTS[v0];
  [1:v]trim=duration=5,setpts=PTS-STARTPTS[v1];
...
## Transition 10: Spin / Rotate Transition

Clip 1 spins and fades into clip 2.
...
Abrupt cut with brief RGB-split glitch frame.
...
Clip 1 blurs out, clip 2 blurs in — dreamy transition.
...
If ffmpeg version supports `xfade`, these are easier:

```bash
# List of xfade transitions:
...
Python script to chain transitions across multiple clips:
...
trans_dur = 1             # seconds
clip_dur = 5              # seconds per clip (after trim)
