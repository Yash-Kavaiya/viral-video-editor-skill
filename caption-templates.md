...
for i in range(1, len(text) + 1):
    ...
    # Last chunk stays visible until END time
    if i == len(text):
...
When burning SRT files with styled subtitles, use the ASS force_style parameter:
...
You can stack multiple drawtext filters to create rich multi-element captions:
