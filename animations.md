...
```
# Requires two inputs. 1-second crossfade at the cut point.
...
```
# Use on an overlay element for pulse effect
...
### 12. Bounce In (text drops and bounces)
```
...
### 13. Bounce Out (reverse bounce, exits up)
```
drawtext=text='BYE':
  fontfile=/usr/share/fonts/truetype/noto/NotoSans-Bold.ttf:
  fontsize=64:fontcolor=white:borderw=3:bordercolor=black:
...
### 14. Typewriter (see caption-templates.md Template 7 for full implementation)
Use Python generator from caption-templates.md.

### 15. Pop Scale (text pops in big then settles)
```
drawtext=text='POP':
  fontfile=/usr/share/fonts/truetype/noto/NotoSans-Bold.ttf:
...
### 20. Float / Hover (gentle up-down oscillation)
```
...
### 21. Wiggle Text (horizontal jitter)
```
...
### 23. Rubber Band (stretch horizontally then snap)
```
...
### 24. Flip In (vertical flip illusion via scale Y trick)
```
drawtext=text='FLIP':
  fontfile=/usr/share/fonts/truetype/noto/NotoSans-Bold.ttf:
...
### 25. Horizontal Slide Left (whole video pans left)
```
...
### 28. Diagonal Slide (top-left to bottom-right)
```
...
### 31. Glitch Horizontal (horizontal displacement)
```
# Random horizontal shifts — simulated glitch
crop=w=iw-30:h=ih:x='15+15*sin(t*80)*if(gt(random(0),0.85),1,0)':y=0,scale=1080:1920
```

### 32. Glitch RGB Split
```
# Red-blue channel offset
split=3[r][g][b];
...
### 37. Rotate 360 (full spin)
```
rotate='2*PI*t/3:c=none:ow=1080:oh=1920'
```
`t/3` = completes one rotation in 3 seconds.

### 38. Rotate Wiggle (oscillating rotation)
```
...
### 41. Blur In (starts blurry, becomes sharp)
```
avgblur='if(lt(t,1.5),ceil(30*(1-t/1.5)),0)':planes=0xF
```

### 42. Blur Out (sharp to blurry)
```
avgblur='if(gt(t,END-1.5),ceil(30*(t-END+1.5)/1.5),0)':planes=0xF
```

### 43. Radial Blur (zoom blur effect)
```
# Simulate with multiple slight zooms blended
split[a][b];
...
### 45. Color Flash (tint the whole frame briefly)
```
# Orange flash at t=2 for 0.2s
...
```
# Flash to grayscale at t=3 for 0.3s
...
### 47. Split Screen Slide (two videos side by side sliding in)
```
...
### 48. Picture-in-Picture (small video overlay)
```
[1:v]scale=300:-1,setpts=PTS-STARTPTS[pip];
[0:v][pip]overlay=W-w-30:30:enable='between(t,START,END)'
```

### 49. Parallax Layers (foreground moves faster than background)
```
...
### 50. Countdown Timer (3-2-1 text overlay)
```
